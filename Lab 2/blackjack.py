...
class MCTSPlayer(Player):
    ...
    def get_action(self, cards, actions, dealer_cards):
        ...
        for i in range(MCTS_N):#note this is looped 100 times
            p.reset()
            #print(get_value(cards))
            res = g1.continue_round(cards, dealer_cards, self.bet)
            #print(cards)
            #print(get_value(cards))
            act = p.actions[0]
            p.myTree.currentNode.values.append(res)##will have to back propogate here
            if act not in results:
                results[act] = []
            results[act].append(res)
        """    
...
class RolloutPlayer(Player):
    ...
    def initTree(self,cards,dealer_cards):
            ...
            ##self.myTree.rootNode.printNode()
    def get_action(self, cards, actions, dealer_cards):
        """Expand unexpanded"""
        tree = self.myTree
        ...
        """Initial actions only- Priority system"""
        if len(cards) == 2 and same_value(cards[0], cards[1]):
            splitAvail= True
        if tree.rootNode.splitNode is None and splitAvail:
            ...
            tree.currentNode.parent = tree.rootNode #update stand nodes parent
            tree.currentNode.timesVisited = tree.currentNode.timesVisited + 1
            tree.currentNode.isLeaf = True
            act = Action.SPLIT
            self.actions.append(act)
            return act
        if tree.rootNode.standNode is None:
            tree.rootNode.standNode = MCTSNode(tree.rootNode.pcards,tree.rootNode.dcards) #make a new node
            tree.currentNode = tree.rootNode.standNode #update current node selection            
            tree.currentNode.parent = tree.rootNode #update stand nodes parent
            tree.currentNode.timesVisited = tree.currentNode.timesVisited + 1
            tree.currentNode.isLeaf = True
            act = Action.STAND
            self.actions.append(act)
            return act
        if tree.rootNode.ddNode is None:
            tree.rootNode.ddNode = MCTSNode(tree.rootNode.pcards,tree.rootNode.dcards) #make a new node
            tree.currentNode = tree.rootNode.ddNode #update current node selection            
            tree.currentNode.parent = tree.rootNode #update stand nodes parent
            tree.currentNode.timesVisited = tree.currentNode.timesVisited + 1
            tree.currentNode.isLeaf = True
            act = Action.DOUBLE_DOWN
            self.actions.append(act)
            return act
        if tree.rootNode.hitNode is None:
            tree.rootNode.hitNode = MCTSNode(tree.rootNode.pcards,tree.rootNode.dcards) #make a new node
            tree.currentNode = tree.rootNode.hitNode #update current node selection            
            tree.currentNode.parent = tree.rootNode #update stand nodes parent
            tree.currentNode.timesVisited = tree.currentNode.timesVisited + 1
            act = Action.HIT
            self.actions.append(act)
    ...
    def reset(self):
        ...
        if self.myTree.currentNode.isLeaf:#if we are not on root or hit node go up
            self.myTree.currentNode = self.myTree.rootNode 
        
    def selection(self):
        c = 15
        current = self.myTree.rootNode
        hit = current.hitNode
        stand = current.standNode
        dd = current.ddNode
        N = hit.timesVisited+stand.timesVisited+dd.timesVisited
        if current.splitNode != None:
            N = N + current.splitNode.timesVisited
        ...
        if max is hitUCB:    
            ...
            self.myTree.currentNode = hit
            return Action.HIT
        if max is standUCB:    
            stand.timesVisited = stand.timesVisited + 1
            self.myTree.currentNode = stand
            return Action.STAND
        if max is ddUCB:
            dd.timesVisited = dd.timesVisited + 1
            self.myTree.currentNode = dd
